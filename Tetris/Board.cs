...
namespace Tetris
{
    ...
    public class HighScoreBoard : Scoreboard
    {
        ...
        {
            highScorers = File.ReadAllLines("highScorers.txt");
            string[] highScoresText = File.ReadAllLines("highScores.txt");
        ...
        public void updateScores(int currentScore, int x, int y)
        {
            ...
            if (isAWinner)
            {
                File.WriteAllLines("highScorers.txt", highScorers);
                string[] textScores = new string[10];
                for(int i = 0; i < highScores.Length; i++)
                {
                    textScores[i] = Convert.ToString(highScores[i]);
                }
                File.WriteAllLines("highScores.txt", textScores);
            }
        }
    }
